...
public class AutofitHelper {
    ...
    public static AutofitHelper create(TextView view, AttributeSet attrs, int defStyle) {
        ...
        if (attrs != null) {
            ...
            TypedArray ta = context.obtainStyledAttributes(
                    attrs,
                    R.styleable.AutofitScrollTextView,
                    defStyle,
                    0);
            sizeToFit = ta.getBoolean(R.styleable.AutofitScrollTextView_sizeToFit, sizeToFit);
            minTextSize = ta.getDimensionPixelSize(R.styleable.AutofitScrollTextView_minTextSize,
                    minTextSize);
            precision = ta.getFloat(R.styleable.AutofitScrollTextView_precision, precision);
            ta.recycle();
    ...
    private static void autofit(TextView view, TextPaint paint, float minTextSize, float maxTextSize,
        ...
        //now fit the number of line, now try to fit heigh Tung
//        int targetHeight = view.getHeight() - view.getPaddingTop() - view.getPaddingBottom();
//        size = Math.min(size, getAutofitTextHeight(text, paint, targetHeight, size, maxLines,
//                displayMetrics));
     ...
     * @return
     */
    private static float getAutofitTextHeight(CharSequence text, TextPaint paint, int targetHeight
            , float textsize, int lines, DisplayMetrics displayMetrics) {

        paint.setTextSize(TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_PX, textsize,
                displayMetrics));
        Rect bounds = new Rect();
        paint.getTextBounds(text + "", 0, 2, bounds);
       // System.out.println("Auto"+text+":"+targetHeight+":"+textsize+":"+(bounds.height()*lines));
        return textsize*targetHeight/(lines*bounds.height());

    }

    /**
     * Recursive binary search to find the best size for the text. Just width and lines
     */
