 ...
 */
public class AutofitScrollTextView extends TextView implements AutofitHelper.OnTextSizeChangeListener {

    private AutofitHelper mHelper;
    private TimerTask scrollTimerTask;
    private Timer timer;

    public AutofitScrollTextView(Context context) {
        super(context);
        init(context, null, 0);
    }

    public AutofitScrollTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs, 0);
    }

    public AutofitScrollTextView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) {
        ...
        scrollTimerTask = new TimerTask() {
            ...
            public void run() {
                ...
                int scrollAmount = 0;
                try {
                    scrollAmount = getLayout().getLineTop(AutofitScrollTextView.this.getLineCount()) - AutofitScrollTextView.this.getHeight();
                } catch (Exception e) {
                    e.printStackTrace();
                }
                Log.d(AutofitScrollTextView.class.toString(), "scroll " + scrollAmount);
                if (scrollAmount > 0) {
                    AutofitScrollTextView.this.scrollTo(0, scrollAmount / 100 * (i++));
                    i %= 100;
                    if (i == 0) {
                        timer.cancel();
                    }
                } else {
                    AutofitScrollTextView.this.scrollTo(0, 0);
                    timer.cancel();
                }
            }
        };
        ...
        timer = new Timer();
        timer.scheduleAtFixedRate(scrollTimerTask, 500, 100);
    }
